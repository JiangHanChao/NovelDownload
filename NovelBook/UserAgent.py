...
def CheckLog(Logpath):
	...
	if not os.path.isfile(Logpath):
		print ("Error : Not find log file in "+str(Logpath))
		return -1
	log = open(Logpath,'r',encoding='utf-8')  #write and read
	loglist = log.readlines()
	log.close()
	for line in loglist:
		if line.find("Missing Chapter Error") != -1:
			LackChapterPath = line.split(">>")[1]
			Lackchapters.append(LackChapterPath)
	if Lackchapters == []:
		return -1
	return Lackchapters
	...
	if not proxy_flag:	#not use agency
		...
		if try_time<count_time:
			try:
				...
				print('http status code: '+str(book_html.status_code))
